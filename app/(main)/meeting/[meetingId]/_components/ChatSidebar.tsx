...
const ChatSidebar = ({
  ...
  const SidebarContent = (
    <>
      <div className="p-4 border-b border-border bg-card w-full z-10 hidden md:block">
        <h3 className="font-semibold text-foreground">Meeting Assistant</h3>
        <p className="text-sm text-muted-foreground">
          Ask me anything about this meeting
        </p>
      </div>
      <div className="px-2 mt-20 h-[calc(80vh-5px)] md:h-[calc(80vh-60px)] overflow-y-auto pb-24">
      ...
      <div className="w-full flex absolute bottom-0 right-0 p-2">
        <InputGroup>
          <InputGroupTextarea
            placeholder={
  ...
  return (
    <>
      {width && width <= 768 ? (
        <Sheet open={chatSidebarOpen} onOpenChange={setChatSidebarOpen}>
          <SheetContent side="right" className="p-0 w-4/5">
            <SheetHeader className="p-4 border-b border-border">
              <SheetTitle>Meeting Assistant</SheetTitle>
              <SheetDescription>
                Ask me anything about this meeting
              </SheetDescription>
            </SheetHeader>
        ...
        <div
          className={cn(
            chatSidebarOpen ? "w-0" : "w-1/3",
            "overflow-hidden hidden md:block transition-all duration-300"
          )}
        >
          <div
            className={cn(
              chatSidebarOpen ? "translate-x-full" : "translate-x-0",
              "w-full h-full border-l border-border lg:flex flex-col relative transition-transform duration-300"
            )}
          >
            {SidebarContent}
          </div>
